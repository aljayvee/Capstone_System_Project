...
async function probe() {
  console.log("Probing MariaDB credentials...");
  for (const cfg of configs) {
    try {
      ...
      try {
        const [users] = await conn.query('SELECT username, role, email FROM errand_system_db.users;');
        console.log('Users count in errand_system_db:', users.length);
        console.log('Sample users:', users);
      } catch (err) {
        console.log('errand_system_db or users table not found:', err.message);
      }
  ...
  console.log("Could not connect with probed credentials.");
}
