...
// Reads the sidebar_state cookie written by setOpen() below, so a collapsed
// sidebar stays collapsed across a full page reload instead of always
// re-expanding (this is a Vite SPA, not Next.js SSR, so nothing reads the
// cookie back on mount unless we do it explicitly here).
    ...
    >
      ...
      >
        <div
          ...
          className="flex size-full flex-col bg-(--sidebar-background) text-white group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
        >
          {children}
        </div>
      </div>
